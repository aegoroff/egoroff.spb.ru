	...
	Redactor.prototype = {
			...
			if (this.opts.cleanUp === true)
			{
				...
				{ 
					setTimeout($.proxy(function ()
					{
						...
						var marker_text = '';
						if ($.support.mozilla) marker_text = '&nbsp;';
		...
		shortcuts: function()
		{
			$(this.doc).keydown($.proxy(function(e)
			{
				...
				if ($.support.webkit && navigator.userAgent.indexOf('Chrome') === -1)
				{
		...
		observeImages: function()
		{
			...
			{
				if ($.support.msie) $(s).attr('unselectable', 'on');
				this.resizeImage(s);
		...
		execCommand: function(cmd, param)
		{
			if (this.opts.visual && this.doc)
			{
				try
				{
					...
					if (cmd === 'inserthtml' && $.support.msie)
					{
						this.doc.selection.createRange().pasteHTML(param);
					}
					else if (cmd === 'formatblock' && $.support.msie)
					{
		...
		fileUploadCallback: function(json)
		{
			...
			// chrome fix
			if ($.support.webkit && !!window.chrome)
			{
				link = link + '&nbsp;'; 
			}
